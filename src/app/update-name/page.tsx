    ...
    <main className="min-h-screen flex items-center justify-center p-4 bg-gray-50">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] sm:p-8 p-6 space-y-8">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Update Item Name</h1>
          <p className="text-gray-500 text-sm">Scan a barcode or enter the item ID to update its name.</p>
        </div>

        {success && (
          <div className="bg-green-50 text-green-800 border border-green-200 rounded-xl p-4 flex items-center space-x-3">
            <CheckCircle2 className="text-green-500 w-5 h-5 flex-shrink-0" />
            <p className="text-sm font-medium">Item name updated successfully!</p>
          </div>
        )}
        ...
        <div className="space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={!itemId.trim() || !newName.trim() || loading}
            ...
            >
              <span>{loading ? "Updating..." : "Update Name"}</span>
