...
export default function TestPriceUpdateScreen() {
  const [itemId, setItemId] = useState("733599125013");
  const [retail1, setRetail1] = useState("10.99");
  ...
  const [customPayloadString, setCustomPayloadString] = useState(defaultPayload);
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    const finalUrl = `${activeBaseUrl}/api/batch/itemPricing?apikey=${apiKey}&${customParams}`;
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] sm:p-8 p-6 space-y-8 h-fit">
          <div className="text-center space-y-2">
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Test Price Update</h1>
            <p className="text-gray-500 text-sm">Update item price for testing.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Retail Price (retail1)</label>
              <input
                type="text"
                value={retail1}
                onChange={(e) => setRetail1(e.target.value)}
                className="block w-full px-4 py-3 border border-gray-200 rounded-2xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium text-lg bg-gray-50/50"
            ...
            <button
              type="submit"
              disabled={(!useCustomPayload && (!itemId || !retail1)) || loading}
               ...
               <div className="grid grid-cols-1 gap-3">
                 ...
                 <div className="space-y-1.5 flex flex-col pt-2 border-t border-gray-700">
                   ...
                   {useCustomPayload ? (
                      ...
                      <div className="text-[10px] text-gray-500 italic">Using form details (Item ID & Price). Check box to type manual JSON.</div>
                   )}
                 </div>
               </div>
            </div>
